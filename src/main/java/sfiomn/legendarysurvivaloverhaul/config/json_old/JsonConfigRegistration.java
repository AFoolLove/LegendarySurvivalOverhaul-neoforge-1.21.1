		...
		if (jsonDimensionTemperatures != null)
		{
			...
			if (!diff.entriesOnlyOnRight().isEmpty() || !diff.entriesDiffering().isEmpty()) {
				...
				for (String e: entriesToKeep) {
					ResourceLocation rl = ResourceLocation.withDefaultNamespace(e);
					...
					if (!rl.equals(ResourceLocation.withDefaultNamespace("the_nether")) && !rl.equals(ResourceLocation.withDefaultNamespace("the_end"))) {
				...
				for (String e: entriesToKeep) {
					...
					for (JsonBlockFluidThirst jbft: JsonConfig.blockFluidThirst.get(e)) {
						...
						for (JsonEffectParameter jep: jbft.effects) {
							MobEffect me = BuiltInRegistries.MOB_EFFECT.get(ResourceLocation.withDefaultNamespace(jep.name));
							if (me != null)
