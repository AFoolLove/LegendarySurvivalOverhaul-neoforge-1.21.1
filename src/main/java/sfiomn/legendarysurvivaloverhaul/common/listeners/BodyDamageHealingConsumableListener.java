...
public class BodyDamageHealingConsumableListener extends SimpleJsonResourceReloadListener implements IHealingConsumableManager {
    ...
    protected void apply(@NotNull Map<ResourceLocation, JsonElement> resourceLocationJsonElementMap, @NotNull ResourceManager resourceManager, @NotNull ProfilerFiller profilerFiller) {
        ...
        resourceLocationJsonElementMap.forEach((key, json) -> {
            try {
                DataResult<JsonHealingConsumable> parsedJson = JsonHealingConsumable.CODEC.parse(JsonOps.INSTANCE, json);
                JsonHealingConsumable temperatures = parsedJson.getOrThrow(error -> {
                    LegendarySurvivalOverhaul.LOGGER.error("Failed parsing body healing consumable : {}", error);
